...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Failed to prepare the Node.js license file." }

    & npm exec -- esbuild "src/server.cjs" --bundle --platform=node --format=cjs --target=node24 --outfile="build/app.cjs"
    if ($LASTEXITCODE -ne 0) { throw "Failed to bundle the application." }

    & node --experimental-sea-config "sea-config.json"
    if ($LASTEXITCODE -ne 0) { throw "Failed to generate the SEA blob." }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Failed to remove the original Node.js signature." }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Failed to inject the application into the executable." }
